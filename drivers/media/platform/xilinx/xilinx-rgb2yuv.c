...
static int xrgb2yuv_s_stream(struct v4l2_subdev *subdev, int enable)
{
	...
	xvip_set_frame_size(&xrgb2yuv->xvip, width, height);
...
/**
 * xrgb2yuv_init_format - Initialize formats on all pads
 * @subdev: rgb2yuv V4L2 subdevice
 ...
 */
static void xrgb2yuv_init_format(struct v4l2_subdev *subdev,
			     struct v4l2_subdev_fh *fh)
{
	struct xrgb2yuv_device *xrgb2yuv = to_rgb2yuv(subdev);
	struct v4l2_mbus_framefmt *__format;
	...
	__format = __xrgb2yuv_get_pad_format(xrgb2yuv, fh, XVIP_PAD_SINK,
					     which);
...
{
	xrgb2yuv_init_format(subdev, fh);
...
static int xrgb2yuv_probe(struct platform_device *pdev)
{
	...
	xvip_get_frame_size(&xrgb2yuv->xvip, &default_format->width,
	...
	*default_format = xrgb2yuv->default_formats[XVIP_PAD_SINK];
	...
	xrgb2yuv_init_format(subdev, NULL);
