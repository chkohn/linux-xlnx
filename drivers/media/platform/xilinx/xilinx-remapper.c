...
__xremap_get_pad_format(struct xremap_device *xremap, struct v4l2_subdev_fh *fh,
			     ...
			     struct v4l2_subdev_format *fmt)
{
	...
	fmt->format = *__xremap_get_pad_format(xremap, fh, fmt->pad,
					       fmt->which);
			     ...
			     struct v4l2_subdev_format *fmt)
{
	...
	format = __xremap_get_pad_format(xremap, fh, fmt->pad, fmt->which);
	...
	format = __xremap_get_pad_format(xremap, fh, XREMAP_PAD_SOURCE,
					 fmt->which);
	*format = fmt->format;
...
static int xremap_pm_prepare(struct device *dev)
{
	struct xremap_device *xremap = dev_get_drvdata(dev);

	return 0;
}

static int xremap_pm_suspend(struct device *dev)
{
	struct xremap_device *xremap = dev_get_drvdata(dev);
...
static int xremap_pm_resume(struct device *dev)
{
	struct xremap_device *xremap = dev_get_drvdata(dev);
...
static void xremap_pm_complete(struct device *dev)
{
	struct xremap_device *xremap = dev_get_drvdata(dev);
}
...
#define xremap_pm_prepare	NULL
#define xremap_pm_suspend	NULL
#define xremap_pm_resume	NULL
#define xremap_pm_complete	NULL
...
static const struct dev_pm_ops xremap_pm_ops = {
	.prepare = xremap_pm_prepare,
	.suspend = xremap_pm_suspend,
	.resume = xremap_pm_resume,
	.complete = xremap_pm_complete,
};
