...
/*
 * Private Controls for Xilinx CCM Video IP
 */

#define V4L2_CID_XILINX_CCM			(V4L2_CID_USER_BASE + 0xb000)
...
static int xccm_s_stream(struct v4l2_subdev *subdev, int enable)
{
	...
	xvip_set_frame_size(&xccm->xvip, &xccm->format);
...
static int xccm_open(struct v4l2_subdev *subdev, struct v4l2_subdev_fh *fh)
{
	...
	*v4l2_subdev_get_try_format(fh, XVIP_PAD_SINK) = xccm->default_format;

	*v4l2_subdev_get_try_format(fh, XVIP_PAD_SOURCE) = xccm->default_format;
...
static int xccm_probe(struct platform_device *pdev)
{
	...
	xvip_get_frame_size(&xccm->xvip, &xccm->default_format);

	/* Initialize the active format with default one */
	xccm->format = xccm->default_format;
