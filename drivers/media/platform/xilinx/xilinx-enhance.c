...
static int xenhance_s_stream(struct v4l2_subdev *subdev, int enable)
{
	...
	xvip_set_frame_size(&xenhance->xvip, &xenhance->format);
...
static int xenhance_probe(struct platform_device *pdev)
{
	...
	xvip_get_frame_size(&xenhance->xvip, &xenhance->default_format);

	/* Initialize the active format */
	xenhance->format = xenhance->default_format;
