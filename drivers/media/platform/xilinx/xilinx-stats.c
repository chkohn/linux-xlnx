...
static int xstats_s_stream(struct v4l2_subdev *subdev, int enable)
{
	...
	xvip_set_frame_size(&xstats->xvip, &xstats->format);
...
static int xstats_probe(struct platform_device *pdev)
{
	...
	xvip_get_frame_size(&xstats->xvip, &xstats->default_format);

	/* Initialize the active format with default one */
	xstats->format = xstats->default_format;
