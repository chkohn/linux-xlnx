...
static int xgamma_s_stream(struct v4l2_subdev *subdev, int enable)
{
	...
	xvip_set_frame_size(&xgamma->xvip, &xgamma->format);
...
static int xgamma_probe(struct platform_device *pdev)
{
	...
	xvip_get_frame_size(&xgamma->xvip, &xgamma->default_format);

	/* Initialize the active format */
	xgamma->format = xgamma->default_format;
