...
static int xtpg_s_stream(struct v4l2_subdev *subdev, int enable)
{
	...
	xvip_write(&xtpg->xvip, XTPG_BOX_COLOR, 0x76543210 & (~0xff << (0 /* tpg_modifier */ % 4 * 8)));
...
static int xtpg_pm_prepare(struct device *dev)
{
...
static void xtpg_pm_complete(struct device *dev)
{
	struct xtpg_device *xtpg = dev_get_drvdata(dev);
}
...
#define xtpg_pm_prepare	NULL
#define xtpg_pm_suspend	NULL
#define xtpg_pm_resume	NULL
#define xtpg_pm_complete	NULL
...
static const struct dev_pm_ops xtpg_pm_ops = {
	.prepare = xtpg_pm_prepare,
	.suspend = xtpg_pm_suspend,
	.resume = xtpg_pm_resume,
	.complete = xtpg_pm_complete,
};
