			 ...
			 unsigned int pad, u32 which)
{
	...
	case V4L2_SUBDEV_FORMAT_TRY:
		return v4l2_subdev_get_try_format(&xscaler->xvip.subdev,
						  cfg, pad);
...
__xscaler_get_crop(struct xscaler_device *xscaler,
		   struct v4l2_subdev_pad_config *cfg,
		   u32 which)
{
	switch (which) {
	case V4L2_SUBDEV_FORMAT_TRY:
		return v4l2_subdev_get_try_crop(&xscaler->xvip.subdev,
						cfg, XVIP_PAD_SINK);
	case V4L2_SUBDEV_FORMAT_ACTIVE:
		return &xscaler->crop;
	default:
