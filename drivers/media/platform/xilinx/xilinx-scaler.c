...
static int xscaler_pm_suspend(struct device *dev)
{
...
static int xscaler_pm_resume(struct device *dev)
{
	struct xscaler_device *xscaler = dev_get_drvdata(dev);
...
#define xscaler_pm_suspend	NULL
#define xscaler_pm_resume	NULL
...
static const struct dev_pm_ops xscaler_pm_ops = {
	.suspend	= xscaler_pm_suspend,
	.resume		= xscaler_pm_resume,
};
