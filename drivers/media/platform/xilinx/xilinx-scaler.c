...
static void xscaler_set_aperture(struct xscaler_device *xscaler)
{
	...
	xvip_write(&xscaler->xvip, XSCALER_HAPERTURE,
		   (end << XSCALER_APERTURE_SHIFT) |
		   (start & XSCALER_APERTURE_MASK));
			      ...
			      struct v4l2_subdev_format *fmt)
{
	...
	if (fmt->pad == XVIP_PAD_SINK) {
		/* recalculate the cropping region */
		crop = __xscaler_get_crop(xscaler, fh, fmt->which);
		crop->left = 0;
		crop->top = 0;
		crop->width = fmt->format.width;
		crop->height = fmt->format.height;
		xscaler_try_crop(&fmt->format, crop);
	}
...
/**
 * xscaler_init_format - Initialize formats on all pads
 ...
 */
static void xscaler_init_format(struct v4l2_subdev *subdev,
				 struct v4l2_subdev_fh *fh)
{
	struct xscaler_device *xscaler = to_scaler(subdev);
	struct v4l2_mbus_framefmt *__format;
	...
	__format = __xscaler_get_pad_format(xscaler, fh, XVIP_PAD_SINK, which);
	*__format = xscaler->default_formats[XVIP_PAD_SINK];

	__format = __xscaler_get_pad_format(xscaler, fh, XVIP_PAD_SOURCE,
					    which);
...
static int xscaler_probe(struct platform_device *pdev)
{
	...
	default_format->width = size & XSCALER_SIZE_MASK;
	default_format->height = (size >> XSCALER_SIZE_SHIFT) &
				 XSCALER_SIZE_MASK;
	...
	*default_format = xscaler->default_formats[XVIP_PAD_SINK];
	...
	xscaler_init_format(subdev, NULL);
