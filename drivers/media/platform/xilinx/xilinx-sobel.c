...
static int xsobel_pm_prepare(struct device *dev)
{
	struct xsobel_device *xsobel = dev_get_drvdata(dev);

	return 0;
}

static int xsobel_pm_suspend(struct device *dev)
{
	struct xsobel_device *xsobel = dev_get_drvdata(dev);
...
static int xsobel_pm_resume(struct device *dev)
{
	struct xsobel_device *xsobel = dev_get_drvdata(dev);
...
static void xsobel_pm_complete(struct device *dev)
{
	struct xsobel_device *xsobel = dev_get_drvdata(dev);
}
...
#define xsobel_pm_prepare	NULL
#define xsobel_pm_suspend	NULL
#define xsobel_pm_resume	NULL
#define xsobel_pm_complete	NULL
...
static const struct dev_pm_ops xsobel_pm_ops = {
	.prepare = xsobel_pm_prepare,
	.suspend = xsobel_pm_suspend,
	.resume = xsobel_pm_resume,
	.complete = xsobel_pm_complete,
};
