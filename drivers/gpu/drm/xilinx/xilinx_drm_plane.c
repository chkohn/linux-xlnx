			...
			unsigned int possible_crtcs, bool priv)
{
	...
	if (sub_node) {
		...
		fmt_in = xilinx_cresample_get_input_format(plane->cresample);
		ret = xilinx_drm_format(fmt_in, &fmt_in);
		if (ret)
			goto err_dma;
		...
		fmt_out = xilinx_cresample_get_output_format(plane->cresample);
		ret = xilinx_drm_format(fmt_out, &fmt_out);
		if (ret)
			goto err_dma;
	}
