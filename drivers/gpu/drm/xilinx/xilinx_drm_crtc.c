				    ...
				    struct drm_framebuffer *old_fb)
{
	...
	/* set pixel clock */
	clk_set_rate(crtc->pixel_clock, adjusted_mode->clock * 1000);
...
struct drm_crtc *xilinx_drm_crtc_create(struct drm_device *drm)
{
	...
	if (IS_ERR_OR_NULL(crtc->pixel_clock)) {
		DRM_DEBUG_KMS("failed to get pixel clock\n");
		ret = -EPROBE_DEFER;
		goto err_out;
	}
	clk_prepare(crtc->pixel_clock);
	clk_enable(crtc->pixel_clock);
